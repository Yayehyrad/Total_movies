  ...
  return (
    <section className='w-full h-full'>
        <div className='flex min-h-full max-h-[95vh] overflow-hidden'>
            {
                bannerData.map((data : MovieData,index)=>{
                    return(
                                ...
                                <div className='container mx-auto'>
                                    <div className=' w-full absolute bottom-0 max-w-md px-3'>
                                        ...
                                        <div className='flex items-center gap-2 w-full'>
                                            <p>Rating : { Number(data.vote_average).toFixed(1) }<RiStarSFill className=' inline'/> </p>
                                            <span>|</span>
                                            <p>View : { Number(data.popularity).toFixed(0) }</p>
                                        </div>
                                        <Link to={"/"+data?.media_type+"/"+data.id}>
                                            <button  className=' bg-white px-4 py-2 text-black font-bold rounded mt-4  hover:bg-gradient-to-l from-red-700 to-orange-500 shadow-md transition-all duration-1000 hover:scale-105'>
                                                Play Now
                                            </button>
                                        </Link>
                                    </div>
